
<%@ page import="net.sksi.freezerbuddy.ContainedItem" %>
<!DOCTYPE html>
<html>
	<head>
		<meta name="layout" content="main">
		<g:set var="entityName" value="${message(code: 'containedItem.label', default: 'ContainedItem')}" />
		<title><g:message code="default.list.label" args="[entityName]" /></title>
	</head>
	<body>
		<a href="#list-containedItem" class="skip" tabindex="-1"><g:message code="default.link.skip.label" default="Skip to content&hellip;"/></a>
		<div class="nav" role="navigation">
			<ul>
				<li><a class="home" href="${createLink(uri: '/')}"><g:message code="default.home.label"/></a></li>
				<li><g:link class="create" action="create"><g:message code="default.new.label" args="[entityName]" /></g:link></li>
			</ul>
		</div>
		<div id="list-containedItem" class="content scaffold-list" role="main">
			<h1><g:message code="default.list.label" args="[entityName]" /></h1>
			<g:if test="${flash.message}">
				<div class="message" role="status">${flash.message}</div>
			</g:if>
			<table>
			<thead>
					<tr>
					
						<g:sortableColumn property="quantity" title="${message(code: 'containedItem.quantity.label', default: 'Quantity')}" />
					
						<th><g:message code="containedItem.itemDefinition.label" default="Item Definition" /></th>
					
					</tr>
				</thead>
				<tbody>
				<g:each in="${containedItemInstanceList}" status="i" var="containedItemInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
					
						<td><g:link action="show" id="${containedItemInstance.id}">${fieldValue(bean: containedItemInstance, field: "quantity")}</g:link></td>
					
						<td>${fieldValue(bean: containedItemInstance, field: "itemDefinition")}</td>
					
					</tr>
				</g:each>
				</tbody>
			</table>
			<div class="pagination">
				<g:paginate total="${containedItemInstanceCount ?: 0}" />
			</div>
		</div>
	</body>
</html>
